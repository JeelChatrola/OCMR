...
class PidVelocity():
    ...
    def __init__(self):
        rospy.init_node("pid_velocity_L")
        ...
        self.previous = 0
        self.wheel_prev = 0
        self.wheel_current = 0
        self.then = rospy.Time.now()
        self.wheel_mult = 0
        self.prev_encoder = 0
    
        # pid parameters
        self.Kp = rospy.get_param('~Kp',90)
        self.Ki = rospy.get_param('~Ki',7)
        ...
        self.rolling_pts = rospy.get_param('~rolling_pts',10)
        self.timeout_ticks = rospy.get_param('~timeout_ticks',10)
        
        # motor parameters 
        self.ticks_per_meter = rospy.get_param('~ticks_meter',40690)
        self.vel_threshold = rospy.get_param('~vel_threshold',0.2)
        ...
        rospy.Subscriber("encoder_L", Int64, self.wheelCallback) 
        rospy.Subscriber("wheel_vtarget_L", Float32, self.targetCallback) 
        self.pub_motor = rospy.Publisher('pwm_L',Int16, queue_size=10) 
        self.pub_vel = rospy.Publisher('wheel_vel_L', Float32, queue_size=10)

    # infinite loop to keep running the node till ros is shutdown
    def spin(self):
    ...
    # function to calculate the current velocity of motor using encoder data
